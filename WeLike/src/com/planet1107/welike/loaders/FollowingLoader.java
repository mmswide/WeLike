...
public class FollowingLoader extends AsyncTaskLoader<List<User>> {
    ...
    public FollowingLoader(Context context, int userID) {
    ...
    public List<User> loadInBackground() {
    	...
    	int page = mFollowing != null ? (mFollowing.size() / 7 + 1) : 1;
    	ArrayList<User> likes = sharedConnect.getFollowing(mUserID, page, 7);
        return likes;
    }
    ...
    public void deliverResult(List<User> users) {
        ...
        mLoadMore = users != null ? users.size() == 7 : false;
        if (isStarted()) {
